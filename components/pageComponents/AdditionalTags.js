...
function AdditionalTags({flex, num}) {
  return (
    <div className={`flex text-gray-500 flex-${flex} gap-3  ${flex === 'row' ? 'justify-center' : ''}`}>
    {flex === 'col' ? (
        <>
        {num.map((tag,i) => {
            return (
                    <div key={i} className='flex  gap-3    rounded-xl px-4 py-2 first-of-type:mt-6'>
                        <div className='relative h-6 w-6'>
                            <Image src={TAGS[tag].icon} alt='' fill className='object-contain' />
                        </div>
                        <div>
                            <p className='text-black font-semibold'>{TAGS[tag].tagName}</p>
