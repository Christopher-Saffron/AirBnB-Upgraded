...
function ShareSave({link, id}) {
    ...
    useEffect(() => {
        ...
        if (sessionStorage.getItem('likedPlaces').split(',').includes(router.query.id)) {
            toggleLiked(true)
        }
    }, [])
    ...
    return (
        <>
            <div className='flex gap-3 select-none self-end mt-3 md:mt-0'>
                ...
                <div className='flex gap-1 items-center cursor-pointer underline' onClick={likePage}>
                    
                    {liked ? <FilledHeartIcon className='h-6 w-6 text-red-500  heartAnim' /> : <HeartIcon className='h-6 w-6' />}
                    <span>Save</span>
                </div>
            </div>
            {shareBox && (
                <div className='fixed  top-0 left-0 z-50 w-screen h-screen' style={{background: 'rgba(0,0,0,0.3)'}} onClick={(e) => {closeShareBox(e)}}>
                    <div className='border  shadow-xl bg-gray-200 flex flex-col rounded-lg  h-fit w-fit -translate-x-1/2 -translate-y-1/2 left-1/2 bottom-1/3 transform absolute fromBottom' onClick={likePage}>
                        <div className='flex flex-col bg-gray-200 p-5 relative' style={{zIndex: 25}}>
                            <div onClick={() => {copyLink(`${window.location.origin}${router.asPath}`)}} className=' py-3 text-xl flex items-center gap-3 cursor-pointer hover:text-red-500 hover:underline underline-offset-4'>
            ...
            {copiedNotif && (
                <div className='copiedNotif' onAnimationEnd={() => {toggleCopiedNotif(false)}}>
                    Link copied!
                </div>
            )}
