...
function Reservation({ info }) {
  ...
  const [total, setTotal] = useState(
    Number(info.price.replace("PLN / night", ""))
  );
  ...
  const [calculatedInfo, setCalculatedInfo] = useState({
    ...
    airBnbService: Math.floor(
      (info.price.replace("PLN / night", "") / 50) * 12
    ),
  ...
  function getNewTotal() {
    const newTotal =
      calculatedInfo.priceDays +
      calculatedInfo.cleaningFee +
      calculatedInfo.airBnbService +
      calculatedInfo.taxes;
  ...
  function calculateDays(startDate, endDate) {
    ...
    const diffDays = {
      ...
      priceDays:
        ...
        daysNumber *
        guestNumber,
      cleaningFee: Math.floor(
        (info.price.replace("PLN / night", "") / 100) * 12
      ),
      airBnbService: Math.floor(
        (info.price.replace("PLN / night", "") / 50) * 12
      ),
  ...
  return (
    ...
    <div className="border p-4 border-gray-300 shadow-xl w-fit mx-auto relative md:sticky md:top-32 rounded-md bg-white reservationBar">
      ...
      <div className="bg-gray-100 p-2 rounded-md shadow-sm border border-gray-200">
        Guests:
        <input
          className="bg-inherit pl-2 text-[#FD5B61] focus:outline-none"
          value={guestNumber}
          onChange={(e) => {
      ...
      <div
        onClick={reserve}
        className="mx-auto cursor-pointer duration-200 hover:scale-105 w-full px-3 text-center font-semibold text-2xl text-white rounded-md mt-4 py-3 bg-gradient-to-r from-[#FD5B61] to-pink-500"
      >
        Reserve
      </div>
      <div className="p-2 flex flex-col gap-3 border-b border-gray-300">
        <div className="flex justify-between">
          <p className="underline">
            {info.price.replace("/ night", "")} x {calculatedInfo.howManyDays}{" "}
            {calculatedInfo.howManyDays === 1 ? "night" : "nights"}
          </p>
