            ...
            <div className='flex flex-col px-3 md:px-0 items-center md:flex-row  justify-center md:items-stretch gap-4'>
              <div className='flex-grow w-full md:max-w-[350px] h-full'>
                <div className='mx-auto max-w-[600px] h-full  font-semibold w-full p-3 border rounded-lg text-sm mt-3 shadow-lg'>
                  <div className='relative h-48 sm:h-64'>
                    <Image src={router.query.img} alt='' fill className='object-cover rounded-lg' />
                  </div>
                  <div className='mt-3'>
                    <div><span className='text-[#FD5B61]'>Place:</span> {router.query.placeName}</div>
                    <div><span className='text-[#FD5B61]'>Host:</span> {router.query.placeHost}</div>
                    <div><span className='text-[#FD5B61]'>First Day:</span> {router.query.dateStart}</div>
                    <div><span className='text-[#FD5B61]'>Last Day:</span> {router.query.dateEnd}</div>
                    <div className='text-base pt-2 mt-2 border-t border-gray-300'>Total: <span className='font-bold'>{router.query.fullPrice} PLN</span> </div>
                  </div>
                </div>
              </div>
              ...
              <div className='flex-grow w-full  md:max-w-[350px]'>
                <div className='mx-auto max-w-[600px] text-[#6772E5] font-semibold w-full p-3 border rounded-lg text-xs my-3 shadow-lg'>
                  ...
                  <p>Email: <span className='text-gray-700 italic'>any</span></p>
                  <p>Card information: 4242 4242 4242 4242</p>
                  <p>04/24 424</p>
                  <p>Name on card: <span className='text-gray-700 italic'>any</span></p>
                </div>
